...
def mega_scan(node_closes: pd.DataFrame, pressure_gauge=None, top: int = 20,
    ...
    regime["only_sectors"] = list(only_sectors) if only_sectors else None
...
def forecast_all(min_n: int = 300, price_floor: float = 3.0,
    ...
    if only_sectors:
        want_s = {str(s) for s in only_sectors}
        tradeable = tradeable & np.array([str(s) in want_s for s in sectors])
    elif hot_only:
        try:
...
def macro_only_scan(regime: str, top: int = 20, strict: bool = True,
                    only_sectors: list | None = None) -> tuple:
    ...
    if only_sectors:
        want = {str(s) for s in only_sectors}
        eligible = eligible & np.array([str(s) in want for s in sectors])
